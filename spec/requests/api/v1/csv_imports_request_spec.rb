...
RSpec.describe 'API V1 CsvImports', type: :request do
  describe 'POST /api/v1/importar_dados' do
    ...
    it 'successfully import data' do
      ...
      expect(response.parsed_body['message']).to eq('Importação bem-sucedida!')
    end
    ...
    it 'returns an error when the file is not sent' do
      ...
      expect(response.parsed_body['error']).to eq('Nenhum arquivo enviado')
    end
    ...
    it 'returns error when file is not CSV' do
      ...
      expect(response).to have_http_status(:unprocessable_entity)
      expect(response.parsed_body['error']).to eq('Formato de arquivo inválido. Por favor, envie um arquivo CSV.')
    end
  end
end
