...
RSpec.describe Api::V1::RetornosController, type: :controller do
  ...
  describe 'GET #index' do
    it 'returns a paginated list of feedbacks' do
      create(:retorno, :valid)
      get :index, params: { page: 1 }

      expect(response).to have_http_status(:ok)
      expect(response.parsed_body['collection'].length).to eq(1)
      expect(response.parsed_body['meta']).to have_key('current_page')
    end
  end
  ...
  describe 'GET #show' do
    it 'returns a specific feedback' do
      ...
      expect(response).to have_http_status(:ok)
      expect(response.parsed_body['id']).to eq(retorno.id)
    end
  ...
  describe 'PUT #update' do
    ...
    context 'with valid attributes' do
      it 'updates the feedback' do
        ...
        expect(response).to have_http_status(:ok)
        expect(response.parsed_body['data_da_resposta']).to eq('2024-12-28')
      end
    end
    ...
    context 'with invalid attributes' do
      it 'does not update the feedback and returns errors' do
        ...
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.parsed_body['interesse_no_cargo']).to include('deve estar entre 1 e 10')
      end
    end
  end
  ...
  describe 'DELETE #destroy' do
    ...
    it 'returns an error if deletion fails' do
      allow_any_instance_of(Retorno).to receive(:destroy).and_raise(StandardError.new)

      delete :destroy, params: { id: retorno.id }

      expect(response).to have_http_status(:unprocessable_entity)
      expect(response.parsed_body['error']).to eq('Erro ao deletar o feedback.')
    end
  end
end
